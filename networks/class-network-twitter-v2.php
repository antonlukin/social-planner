<?php
/**
 * Twitter API V2 network handler for Social Planner plugin
 *
 ...
 * @since 1.2.0
 */
class Network_Twitter_V2 {
	/**
	 * Unique network slug.
	 *
	 * @var string
	 */
	const NETWORK_NAME = 'twitter-v2';
	 ...
	 * @var string
	 */
	const HELPER_LINK = 'https://wpset.org/social-planner/setup/#twitter';

	/**
	 * Return human-readable network label
	 */
	public static function get_label() {
		return _x( 'Twitter API v2', 'provider label', 'social-planner' );
	}
	...
	public static function send_message( $message, $settings ) {
		...
		if ( ! empty( $response->data->id ) ) {
			return "https://twitter.com/_/status/{$response->data->id}";
		}

		if ( ! empty( $response->detail ) ) {
			return new WP_Error( 'sending', $response->detail );
		}
